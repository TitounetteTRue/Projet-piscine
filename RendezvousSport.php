...
<title>Prendre rendez-vous Salle de Sport</title>
...
?><!--Affichage de la legende du calendrier-->
<table id="recap">
	<tr>
...
<?php
/* Prise du choix du rendez vous de sport*/
if(isset($_SESSION['Login'])){
	if(
	...
	isset($_GET['choix']) AND preg_match("#^(0|1)$#",$_GET['choix'])) {
		...
		if($_GET['choix']==1){
			...
			$sql="INSERT INTO calendrier_salle(Id_Client, Jour, Heure)
			VALUES('$id','$jour','$heur')";
			...
			if(mysqli_query($db_handle,"DELETE FROM calendrier_salle WHERE  Id_Client='$id' AND Heure='$heur' AND Jour='$jour'")) {
...
<table style="border:1px solid black;border-collapse:collapse;box-shadow: 10px 10px 5px #888888;">
	...
	<tr>
		...
		for($jour=1;$jour<=7;$jour++) {
			...
			$JourReserve=0;
			$req = mysqli_query($db_handle,"SELECT * FROM calendrier_salle WHERE Heure ='$heur' AND Jour='$jour'");
			...
			<tr>
				...
				<a href="?jour=<?php echo $jour; ?>&amp;heure=<?php echo $heur; ?>&amp; choix=<?php echo $JourReserve==1?0:1; ?>#recap">
...
<br>
            ...
            <a class="link-offset-2 link-underline link-underline-opacity-0 link-opacity-50-hover" href="Nos services.html"> RETOUR A LA PAGE PRECEDENTE</a>
